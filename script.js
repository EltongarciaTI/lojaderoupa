  ...
  carrinho.forEach(item => {
    ...
    div.innerHTML = `
      <div class="cart-item__img">
        <!-- FOTO: imgs/produto-${item.id}.jpg -->
        <img src="imgs/produto-${item.id}.jpg" alt="${item.nome}" class="cart-item__photo" />
      </div>
...
const WHATSAPP_NUMERO = '5511999990000'; // ← troque pelo número real
...
function criarCard(p, opts = {}) {
  ...
  div.innerHTML = `
    <div class="produto-card__img">
      ...
      <img src="imgs/produto-${p.id}.jpg" alt="${p.nome}" class="produto-card__photo" />
      ${badgeHtml}
...
function initProduto() {
  ...
  // Imagem — FOTO: imgs/produto-{id}.jpg
  const fotoEl = document.getElementById('produtoFoto');
  if (fotoEl) {
    fotoEl.src = `imgs/produto-${produto.id}.jpg`;
